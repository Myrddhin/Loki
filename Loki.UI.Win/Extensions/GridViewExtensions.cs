...
namespace Loki.UI.Win
{
    public static class GridViewExtensions
    {
        ...
        public static void BindCommandOnDoubleClick<TModel, TItem>(this GridView view, Expression<Func<TModel, ICommand>> commandGetter)
            where TModel : class
            where TItem : class
        {
        ...
        public static void ConfigureGridCommands<TVM, TItem>(this GridView grid, Expression<Func<TVM, IEnumerable<ICommandElement>>> rowCommands, IObservableCollection<TItem> dataSource)
            where TItem : class
            where TVM : class
        {
            // menu
            if (grid.GridControl != null)
            {
                grid.PopupMenuShowing += (s, e) =>
                {
                    ...
                    if (e.Menu.MenuType == GridMenuType.Row || e.Menu.MenuType == GridMenuType.User || e.HitInfo.HitTest == GridHitTest.EmptyRow)
                    {
        ...
        public static void ExportToExcel(this GridView grid)
        {
            ...
            try
            {
                string tempfile = string.Format("{0}export_excel_{1}.xlsx", Path.GetTempPath(), System.DateTime.Now.Ticks);
